...
ORC_REGISTER_STAGE(RawEFMSinkStage)

// Force linker to include this object file
void force_link_RawEFMSinkStage() {}
...
bool RawEFMSinkStage::trigger(
    ...
    try {
        ...
        ORC_LOG_DEBUG("RawEFMSink: Processing {} fields", total_fields);
        ...
        ORC_LOG_DEBUG("RawEFMSink: Total EFM t-values: {}", total_tvalues);
        ...
        for (FieldID fid = start_field; fid < end_field; ++fid) {
            if (cancel_requested_.load()) {
                ...
                ORC_LOG_WARN("RawEFMSink: {}", last_status_);
        ...
        ORC_LOG_INFO("RawEFMSink: Successfully wrote {} t-values to {}", 
                    tvalues_written, output_path);
        ORC_LOG_DEBUG("RawEFMSink: Expected t-values: {}, Actual t-values: {}, Match: {}", 
                     total_tvalues, tvalues_written, total_tvalues == tvalues_written ? "YES" : "NO");
        
        if (invalid_tvalue_count > 0) {
            ORC_LOG_WARN("RawEFMSink: Found {} invalid t-values (outside range [3, 11])", 
                        invalid_tvalue_count);
        }
